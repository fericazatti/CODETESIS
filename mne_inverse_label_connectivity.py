...
annotation_2_event_id = {'AAA': 1,
                        'BBB': 2,
                        'CCC': 3}

# events from annotations
events2, _ = mne.events_from_annotations(
    raw, annotation_2_event_id)


fig = mne.viz.plot_events(events2, 
event_id=annotation_2_event_id,                         
sfreq=raw.info['sfreq']            
              )
fig = raw.plot(start=2, duration=6)

epochs_test = mne.Epochs(raw=raw, events=events2, 
    event_id= annotation_2_event_id, baseline=None)
...
stcs = apply_inverse_epochs(epochs_test, inverse_operator, lambda2, method,
...
plot_connectivity_circle(con.get_data(output='dense')[:,:,0], label_names,
                         node_colors=label_colors, title='All-to-All Connectivity left-Auditory '
                         'Condition (PLI)')
